...
class HomeScreenSection {
  ...
  HomeScreenSection.fromJson(Map<String, dynamic> json) {
    ...
    if (json['section_data'] != null && json['section_data'] is List) { // Perbaikan: Tambah pengecekan tipe
      sectionData = <ItemModel>[];
      // Pastikan 'v' adalah Map sebelum di-parsing
      (json['section_data'] as List).forEach((v) {
...
class CustomFields {
  ...
  CustomFields.fromJson(Map<String, dynamic> json) {
    ...
    type = json['type'];
    image = json['image'];
    // Periksa apakah 'value' tidak null dan merupakan sebuah List sebelum di-cast.
    if (json['value'] != null && json['value'] is List) {
      value = (json['value'] as List).map((item) => item.toString()).toList();
    } else {
      // Jika null atau bukan list, inisialisasi sebagai list kosong.
      value = [];
    }
  }
